...
没有手动恢复命令。如果某次 sync 无法继续，先用 `cocodex status` 或
`cocodex log` 查看状态，然后让提示中指出的 owner 开发者在自己的
managed worktree 中运行 `cocodex sync`；如果该 Codex session 已关闭，
先运行 `cocodex join <name>` 重新进入。
...
正常 semantic sync task 要求 `join` 在 tmux 中运行，这样 Cocodex 才能粘贴 prompt 并发送 Enter。如果没有可用 tmux target，Cocodex 会拒绝 task 启动，并恢复 session snapshot 以便重试。
...
如果 Git 出现 conflict、留下 unsafe state，或者轻量检查失败，Cocodex 会将 Alice 的 worktree 重置到最新 `main`，在 `.cocodex/tasks/` 下写出 task file，把 sync prompt 粘贴进 Alice 的 Codex 终端并发送 Enter。只有 prompt 注入成功，这次 semantic task 才算被接受；如果无法安全注入 prompt，Cocodex 会把 Alice 的 worktree 恢复到 snapshot，释放 lock，并拒绝这次 `sync`。Alice 的 Codex 再读取 task file，在最新 `main` 上重新实现或语义融合 Alice 的 feature。如果这个 task 到来时 Codex 正在处理另一个开发请求，Codex 应该先选择安全暂停点，保留当前请求剩余意图，完成这个 sync task，并在 sync 成功后继续之前暂停的开发工作。
